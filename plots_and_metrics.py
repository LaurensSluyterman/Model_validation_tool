...
def area_bootstrap_comparison(observations, model_1_predictions, model_2_predictions, B=10000):
    ...
    for _ in range(B):
        ...
        model_1_predictions_resample = np.random.choice(model_1_predictions, size=len(observations), replace=True)
        model_2_predictions_resample = np.random.choice(model_2_predictions, size=len(observations), replace=True)
    ...
    area_1 = np.trapz(np.abs(cdf_1(x_values) - cdf_observations(x_values)), x_values)
    area_2 = np.trapz(np.abs(cdf_2(x_values) - cdf_observations(x_values)), x_values)
    CI_1 = [np.round(np.percentile(areas_1, 5), 2), np.round(np.percentile(areas_1, 95), 2)]
    CI_2 = [np.round(np.percentile(areas_2, 5), 2), np.round(np.percentile(areas_2, 95), 2)]
    ratios = np.array(areas_1) /  np.array(areas_2)
    CI_ratio = [np.round(np.percentile(ratios, 5), 2), np.round(np.percentile(ratios, 95), 2)]
...
def calculate_metrics_comparison(observations, model_1_predictions, model_2_predictions, output=False):
    ...
    GMR_1, CI_GMR_1 = CI_G_LD(geomean_observations, geomean_1, GCV(observations), GCV(model_1_predictions),len(observations), len(model_1_predictions), alpha=0.1) 
    GMR_2, CI_GMR_2 = CI_G_LD(geomean_observations, geomean_2, GCV(observations), GCV(model_2_predictions),len(observations), len(model_2_predictions), alpha=0.1) 
    W_1, CI_W_1, W_2, CI_W_2, ratio, CI_ratio = area_bootstrap_comparison(observations, model_1_predictions, model_2_predictions, B=10000)    
...
def calculate_metrics_single(observations, model_1_predictions, output=True):
    ...
    geomean_observations = scipy.stats.gmean(observations)
    geomean_1 = scipy.stats.gmean(model_1_predictions)
    GMR, CI_GMR = CI_G_LD(geomean_observations, geomean_1,
                        GCV(observations), GCV(model_1_predictions),
                        len(observations), len(model_1_predictions),
                        alpha=0.1) 
  
    mean_area, CI_area = area_bootstrap_single(observations, model_1_predictions, B=10000)
...
def plot_comparison(observations, model_1_predictions, model_2_predictions, xlabel):
    ...
    ax2.fill_between(x_values, cdf_1(x_values), cdf_observations(x_values), color='blue', alpha=0.25)
    ax2.set_ylim([0, 1.01])
    ax2.set_ylabel("CDF")
