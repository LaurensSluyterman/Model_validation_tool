...
from plots_and_metrics import calculate_metrics_single, plot_single, calculate_metrics_comparison, plot_comparison

### UI ###
app_ui = ui.page_fluid(
    ui.h2("Wasserstein distance calculator"),
    ui.input_select("user_choice", "I want to:", choices=["Evaluate a single model", "Compare two models"]),
...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def conditional_ui():
        ...
        if input.user_choice() == 'Evaluate a single model':
            return ui.div(
                ...
                ui.input_file("predictions_model_1", "Predictions", accept=[".csv"], multiple=False),
        ...
        if input.user_choice() == 'Compare two models':
            return ui.div(
                ...
                ui.markdown("""
                            ...
                            """),
                ui.input_file("observations", "Observations", accept=[".csv"], multiple=False),
                ui.input_file("predictions_model_1", "Predictions Model 1", accept=[".csv"], multiple=False),
                ui.input_file("predictions_model_2", "Predictions Model 2", accept=[".csv"], multiple=False),
                ui.markdown("""
    ...
    def calc_result_single():
        ...
        GMR, CI_GMR, W1, CI_W1 = calculate_metrics_single(model_1_predictions=model_1_predictions, observations=observations)
        return f"""
    ...
    def data_plot_single():
        ...
        if observations is None or model_1_predictions is None:
            return
        plot = plot_single(observations=observations, predictions=model_1_predictions,
                           xlabel='Value')
        return plot
    ...
    def calc_result_comparison():
        ...
        GMR_1, CI_GMR_1, GMR_2, CI_GMR_2, W_1, CI_W_1, W_2, CI_W_2, ratio, CI_ratio = calculate_metrics_comparison(observations=observations,
                                                                               model_1_predictions=model_1_predictions,
                                                                               model_2_predictions=model_2_predictions, output=True)
        return f"""
    ...
    def data_plot_comparison():
        ...
        if observations is None or model_1_predictions is None or model_2_predictions is None:
            return
        plot = plot_comparison(observations=observations,
                               model_1_predictions=model_1_predictions,
